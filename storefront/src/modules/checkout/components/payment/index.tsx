...
const adyenClientKey = process.env.NEXT_PUBLIC_ADYEN_CLIENT_KEY
...
const Payment = ({
  ...
  const isAdyen = activeSession?.provider_id.startsWith("pp_adyen")
  ...
  const config = useMemo(() => {
    return {
      ...
      onPaymentCompleted: async (result: any, component: any) => {
        await sdk.client
          .fetch(
            `${BACKEND_URL}/store/payment-collections/${cart.payment_collection?.id}/payment-sessions/${activeSession?.id}`,
            {
              method: "POST",
              headers: {
                "x-publishable-api-key":
                  "pk_a95437d6e0cc482966cd9704a8cf9b3950dc8e220740c412f245c571d82398a2",
              },
              body: {
  ...
  useEffect(() => {
    ...
    if (!activeSession || !paymentContainer.current) {
      return
    }
    ...
    const createCheckout = async () => {
      ...
      if (paymentContainer.current && !ignore) {
        await checkout.create("dropin").mount(paymentContainer.current)
      }
    }
